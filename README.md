...
Proposal generation は、明示的な `memory_compression_candidate` finding を `memory_compress` proposal に、明示的な `skill_lifecycle_candidate` finding を `skill_create` / `skill_delete` / `skill_rename` / `skill_merge` proposal に変換できます。`self_improvement_candidate` event も同じ finding として passthrough します。どちらも `recommendation=approval_required` / `auto_apply=false` のままです。実際の変更は apply-plan と approval gate を通します。
