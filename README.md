...
- built-in memory mutation は `memory` tool の add / replace / remove だけを使って適用する。外部 memory provider は capability policy に解決し、Hindsight の stale/incorrect/duplicate `memory_delete` だけ `hindsight_retain` correction として実行可能。sensitive delete や provider tool 不在時は fail-closed
...
- skill / memory mutation の直接ファイル・DB fallback は使わない。skill mutation は `skill_manage` 経由、built-in memory は `memory` tool 経由。外部 memory provider は provider-native tool 経由に限定し、現時点で実行可能なのは Hindsight の stale/incorrect/duplicate delete を `hindsight_retain` correction として保存する path だけ。sensitive delete と tool 不在 runtime は fail-closed
