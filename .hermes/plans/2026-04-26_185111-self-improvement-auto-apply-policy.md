...
Implementation progress as of 2026-04-27: `config.py` now owns defaults, local config loading, execution mode resolution, mode policy validation, and command capability mapping. `__init__.py` re-exports these names for existing tests and wrapper CLI compatibility.
