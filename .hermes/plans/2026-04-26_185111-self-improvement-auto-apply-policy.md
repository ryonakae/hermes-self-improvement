...
- `apply-approved` validation-only / preview-only core, CLI, and tool path, plus `approval-report --include-previews` aggregation and optional `expected_approval_hash` binding for approval review, with actual approved mutation still closed;
