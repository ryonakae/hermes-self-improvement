...
Implementation progress as of 2026-04-27: `config.py` now owns defaults, local config loading, execution mode resolution, mode policy validation, and command capability mapping. `observer.py` now owns `RuntimeObserver`, telemetry JSONL helpers, redaction, retention pruning, partial hook filtering, and tool-result classification. `analysis.py` now owns telemetry aggregation, finding extraction, and proposal generation/deduplication. `__init__.py` re-exports these names for existing tests and wrapper CLI compatibility.
