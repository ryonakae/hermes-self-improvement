...
- [Pre-release role simplification](2026-05-22-pre-release-role-simplification.md) — removes unreleased compatibility rather than preserving it. Tasks 1-3 are implemented: injected editor loops and synthetic finalizer support were deleted from skill/memory backend source, tests now use constrained-runner/helper coverage for that path, a guard keeps the removed synthetic finalizer name out of active source/tests/docs, and status now prints runtime setup reasons while keeping `setup` as the only init/repair surface. Remaining work is to add `target_resolver` as a thin runtime-private overlay target while keeping resolver mutation-free and planner-owned for create/mutate decisions.
