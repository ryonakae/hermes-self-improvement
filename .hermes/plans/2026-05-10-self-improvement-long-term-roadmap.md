...
- **Duplicate / existing coverage decisions:** around 7.0合目.
  ...
  - Milestone 2 phase 2.1 added a deterministic `coverage_fit` bundle (`exact_duplicate / partial_overlap / reference_only / no_existing_fit`) attached to each maintenance candidate. The bundle carries the matched skill names and evidence count, and is rendered compactly in the planner prompt so the planner can reason about duplicate vs partial vs reference vs uncovered cases without forcing the action.
  - Milestone 2 phase 2.2 propagates `maintenance_action` (patch / merge) and the merge `target_skill` from the planner decision into the skill_agent task and prompt. The skill_agent now sees `maintenance_action: patch|merge` (plus `target_skill: <name>` for merge) as an explicit instruction line and as fields in the program-owned task summary, so the patch/merge sub-action is wired end-to-end through the existing native skill patch harness without inventing a new mutation lane. Merge/archive preview semantics (phase 2.3) is still pending.
