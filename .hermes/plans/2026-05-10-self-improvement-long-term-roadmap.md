...
- Follow-up `2026-05-31-memory-to-skill-source-cleanup-execution.md` is implemented and verified: canonical normalized `memory_to_skill` transactions using `target_id`, `source_id`, and `editor_task` now execute like the pre-normalized shape. Mutating execution patches the destination skill first, removes the source memory only after validated skill success, and reports `removed_memories` using the source id. If the skill patch fails, the memory tool is not called and the source memory remains intact. Verification: related suite `85 passed`, full suite `928 passed, 2 skipped`. Latest source dry-run `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260531T001824Z.json` stayed dry-run/no mutation but showed planner variance (`planner_decision_count=21`, `default_defer_count=4`, including `likely_memory_to_skill=3`), so planner consistency remains an observation point.
