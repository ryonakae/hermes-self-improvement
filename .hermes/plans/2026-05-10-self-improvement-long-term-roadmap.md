...
- **Duplicate / existing coverage decisions:** around 6.4合目.
  ...
  - Milestone 2 phase 2.1 added a deterministic `coverage_fit` bundle (`exact_duplicate / partial_overlap / reference_only / no_existing_fit`) attached to each maintenance candidate. The bundle carries the matched skill names and evidence count, and is rendered compactly in the planner prompt so the planner can reason about duplicate vs partial vs reference vs uncovered cases without forcing the action. Patch/merge/archive executor wiring (phases 2.2 / 2.3) is still pending.
