...
[1] Observe real Hermes sessions, tool calls, outcomes, user corrections, skill/memory usage
  ...
  -> [3] Plan knowledge placement and target resolution: classify evidence as skill, builtin_user, builtin_memory, unresolved, none
  -> [4] Produce bounded knowledge transactions with LLM judgment; planner may inspect allowed context but has no mutation tools
  -> [5] Execute through the editor only: official skill and memory tools, including mixed memory-to-skill transactions with add-before-remove semantics
...
- Completion plan `2026-05-28-unified-knowledge-planner-editor-execution.md` is implemented through dry-run readiness closure: `run_improve` uses `run_knowledge_improvement_step`, one planner digest spans skill/memory/user/external-memory evidence, canonical transactions cover every supported knowledge store in deterministic integration proof, unified editor execution is wired, split-lane artifact leakage is removed, and latest dogfood dry-run stayed safely no-op.
- Follow-up `2026-05-30-memory-inventory-planner-dispatch-fix.md` is implemented and verified for the current-memory inventory dispatch regression discovered after the unified inventory slices: `run_improve()` now feeds current built-in entries into `build_evidence_pack()`, and memory placement evidence is no longer counted as skill-only `skill_target_missing`. This was a narrow handoff/accounting fix and regression-proof slice, not a new architecture.
- Follow-up `2026-05-30-memory-inventory-actionability-prompt.md` is implemented and verified: grouped memory inventory cleanup evidence is rendered in the planner prompt, and the latest source dry-run produced explicit skip/no-op decisions for all 3 grouped memory findings instead of omitting them. No safe memory mutation was selected from the current live evidence.
- Follow-up `2026-05-30-memory-inventory-action-hints.md` is implemented and verified: grouped memory inventory findings now carry deterministic action hints into planner digest/prompt (`suggested_action`, `action_reason`, and bounded `memory_operation_hint` when clear). The latest source dry-run stayed no-op because all current grouped memory findings have defer-leaning reasons, not because the planner lacks visibility.
- Follow-up `2026-05-30-memory-placement-planner-actionability.md` is implemented and verified: placement candidates are now first-class planner digest/prompt items with deterministic advisory route hints, an explicit per-candidate prompt requirement, safe default-defer fallback for omitted candidates, and `memory_placement_actionability` accounting. Source dry-run `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260530T161349Z.json` stayed no-op (`target_changed=False`) and exposed 25 placement candidates with `unhandled_count=0` via default defers. Remaining gap: the live planner still did not make semantic placement choices, so any next slice should target planner compliance rather than execution safety.
- Follow-up `2026-05-31-memory-placement-planner-compliance.md` is implemented and verified: the planner prompt now gives exact per-candidate placement transaction templates for move / keep-skip / defer, `decision=move_user_to_memory` / `decision=move_memory_to_user` canonicalize to placement moves, placement decisions citing `source_id` suppress fallback defers, and actionability accounting separates planner semantic choices from default fallback handling (`planner_decision_count` vs `default_defer_count`). Source dry-run `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260530T162808Z.json` stayed dry-run/no mutation and improved placement compliance from all-fallback to `planner_decision_count=19`, `default_defer_count=6`, `unhandled_count=0`, with no duplicate placement handlers. Safety and memory execution behavior are unchanged.
- Follow-up `2026-05-31-memory-placement-default-defer-diagnostics.md` is implemented and verified: default-deferred placement candidates now appear in `default_defer_by_route` and `default_defer_details` with bounded old_text and route reasons. Source dry-run `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260530T164023Z.json` exposed live planner variance (`planner_decision_count=5`, `default_defer_count=20`, `unhandled_count=0`), with omissions concentrated in `likely_keep=16`, `likely_memory_to_skill=3`, `likely_defer=1`. This is diagnostic only; safety and memory execution behavior are unchanged.
- Follow-up `2026-05-31-memory-placement-priority-planner-tuning.md` is implemented and verified: `likely_memory_to_skill` placement candidates now get a priority prompt section, explicit `memory_to_skill` / evidence-preserving defer templates, and priority ordering ahead of low-action `likely_keep` entries. Source dry-run `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260530T222529Z.json` stayed dry-run/no mutation and improved placement compliance to `planner_decision_count=25`, `default_defer_count=0`, `unhandled_count=0`. Safety and memory execution behavior are unchanged.
- Follow-up `2026-05-31-memory-placement-llm-judgment-unblocking.md` is implemented and verified: program-owned `likely_targets` weights were removed from placement evidence, `likely_memory_to_skill` rows now get bounded `candidate_target_skills` as non-command context hints, and run artifacts expose raw planner / normalization diagnostics through `step_decisions.planner_diagnostics` plus bounded `step_decisions.memory_placement_target_hints`. Source dry-run `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260531T075000Z.json` stayed dry-run/no mutation, had 25 placement candidates with `likely_targets=0`, 3 target-hint rows, diagnostics present, and `planner_decision_count=25`, `default_defer_count=0`. Safety and execution behavior are unchanged.
- Follow-up `2026-06-01-memory-placement-cron-hardening.md` is implemented and verified for the latest cron finding: `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260531T190325Z.json` selected two USER↔MEMORY placement applies but both blocked before mutation with `knowledge_transaction_missing_required_fields` because execution required `content` despite exact `source_old_text`. The fix makes placement moves default destination content to `source_old_text`, preserves add-before-remove, validates dry-run apply transactions through the executor boundary, and surfaces blocked apply transactions in reporting. Mutating replay also fixed canonical replay current-entry loading and now blocks stale-source placement moves before destination add. Verified replay `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260601T004540Z.json` had `memory_changes=0`, `skill_changes=0`, no partial placement moves, and all 6 apply attempts safely blocked (`memory_to_skill` missing skill task x3, memory capacity x2, stale source x1). Final verification: related suite `54 passed`; full suite `940 passed, 2 skipped`; `git diff --check` clean.
- Active follow-up `2026-06-01-memory-placement-direction-validation.md` addresses the root planner/normalizer failure found after the cron hardening replay: candidate `memory_place_2217e1ddb538` already had `current_store=user` and `suggested_route=likely_keep`, but raw planner/normalizer output treated USER-shaped content as a `move_memory_to_user` operation and produced an executable-looking `builtin_memory -> builtin_user` placement with empty `evidence_ids`. This plan is not implemented yet, but has passed independent review after one blocker round. It will make allowed move decisions and prompt templates store-specific via shared helpers, fail-close invalid move directions during context-aware normalization, preserve evidence ids for valid placement moves, and expose bounded invalid-direction diagnostics before any executor path is reached.
- Active follow-up `2026-05-29-unified-knowledge-editor-memory-inventory.md` narrows the product gap: current `USER.md` / `MEMORY.md` entries are intended to be first-class planner input, and the same Knowledge Editor must execute skill and memory improvements from one transaction list. This is a simplification pass, not a new safety architecture: no new roles, no approval queue, no confidence gates, and no separate user-visible skill/memory editor lanes.
- Bridge/reporting completion plan: `2026-05-28-knowledge-transaction-unification.md` implemented matched-but-not-selected visibility, canonical `knowledge_transactions` summaries, routed evidence accounting, memory-to-skill bridge execution, normalized `editor_*` errors, and final artifact split-lane removal.

### Important caveat

The role-redesign plan aimed to make **turn trace -> cluster summary -> evidence index/detail -> knowledge transaction plan -> unified editor execution** the canonical model. That model is now implemented through approved mutating replay proof for skill mutation, current `USER.md` / `MEMORY.md` inventory has code support for planner visibility, the planner can express built-in memory placement/cleanup choices as canonical transactions, and the Knowledge Editor can execute those cross-surface transactions through official tools. The 2026-05-30 dispatch-order/accounting regression is fixed, grouped memory inventory findings are visible for explicit planner decisions, and deterministic action hints now explain why current groups lean defer. Improvement is still not proven by one successful skill patch; it requires memory inventory cleanup when evidence is clear and later evidence that repeated workflow/memory mistakes decrease without user correction.
...
- **Runtime-private prompt overlays / GEPA:** around 7.5合目.
  - `calibrate` can generate and promote target_resolver / improvement_planner / skill_agent / memory_agent / evaluator overlay sets.
...
- **Skill quality evaluation:** around 8.4合目.
  - New/updated skills now get deterministic diagnostics for frontmatter, pitfalls, verification, trigger conditions, concrete steps, memory-shaped content, intended patch/edit readback, compactness signals (`content_too_short` / `content_too_long`), and whether the accepted mutation carried attached evidence.
  - Milestone 3 phase 3.1 generates runtime-private evaluator eval cases (`evaluator_skill_quality_{good|needs_patch|too_generic|missing_attached_evidence}_review`) from executed-mutation skill episodes with post-validation signals, so GEPA/DSPy calibration can learn from quality buckets without pinning the deterministic layer to semantic assessments.
  - Milestone 3 phase 3.2 propagates `quality_signals` (`needs_patch` / `missing_sections` / `post_validation_status`) from skill_candidates through the planner digest into a new "Editable skills with quality signals" prompt section, and into the skill_agent task / instructions as a "Quality patch semantics" block. `needs_patch` skills can now become a bounded `mutate_skill (maintenance_action="patch")` candidate limited to the listed `missing_sections`, with no broad rewrite and no automatic retry; a failed patch becomes future outcome evidence instead.
  - Milestone 3 phase 3.3 surfaces the quality cycle in reporting with `Skill quality:` lines `quality patch candidates` (planner-side patch picks) and `quality patched` (skill_agent-accepted patch executions). The existing `skill_quality_needs_patch_penalty` family in outcome_scoring keeps unfinished quality work under observation; a successful low-risk patch with post-validation readback drops the penalty so the next episode can transition from quality hold to improved.
...
- **Knowledge inventory beyond tool failures:** around 7.6合目.
  - Inventory health reports skill overlap/staleness reason counts and memory duplicate/stale-pair counts in CLI/daily-facing summaries, with knowledge-maintenance source buckets (`failure_driven`, `inventory`, `knowledge_coverage`) separating failure-driven proposals from inventory/coverage work.
  - Milestone 4 phase 4.1 enriches `make_skill_inventory_candidate` with deterministic safety metadata: `editable_targets`, `reference_matches`, `evidence_count`, and `recommended_actions` derived from `group_kind` (merge_skills / archive_skill / mutate_skill / no_mutation_target).
  - Milestone 4 phase 4.2 adds deterministic placement routing inside `reconcile_memory_extractor_payload_with_existing_memories`: raw tool output → `skip_reason="not_memory_raw_tool_output"` + `suggested_route="diagnostic"`, procedural / step-shaped facts → `skip_reason="memory_convert_to_skill_update"` + `suggested_route="skill"`.
  - Milestone 4 phase 4.3 introduces `make_skill_drift_candidate` (kind `skill_drift_candidate`, source `inventory`) carrying old_reference / new_reference / confidence / source_paths and an optional failure_trace. The deterministic `mutation_ready` gate requires two independent sources (`two_independent_sources`) or one source plus a failure trace (`authoritative_source_plus_failure_trace`); otherwise the candidate is `mutation_ready=False` with `insufficient_independent_sources`. Phase 4.4 (dogfood / replay verification) is operational and runs in maintenance windows, not as a code change.

- **Outcome / credit assignment:** around 8.5合目.
  - Milestone 5 phase 5.1 surfaces immediate/short/medium/long scoring windows in the compact credit-assignment summary (`outcomes.credit_windows`) and CLI `Outcomes:` section (`scored window coverage`).
  - Phase 5.2 keeps the existing user-correction recurrence path (drop unmatched, stronger negative score than cluster recurrence) under regression tests covering explicit-target match, unrelated-clarification drop, and the stronger-than-cluster scoring contract.
  - Phase 5.3 keeps `target_reedit_shortly_after_mutation` within `REEDIT_WINDOW=7 days` for same target_kind/target_id, with conservative outcome_score `-0.3`, and adds a regression test that later normal edits outside the window do not trigger the signal.
  - Phase 5.4 wires the credit-assignment aggregate into `build_role_runtime_eval_cases`, emitting `evaluator_recurring_outcome_review` / `evaluator_regressed_outcome_review` cases for episodes carrying `post_validation_status`, capped at 30 cases per build, so calibration / GEPA can learn from proven outcome status without letting under-observation signals dominate.
...
2. **One Planner, one Knowledge Editor.** The planner decides across skills, `USER.md`, `MEMORY.md`, none, and memory-to-skill. The editor improves across skills and memory. Tool-specific adapters may exist internally, but user-facing reports/plans should not describe separate skill/memory editor roles.
3. **Minimal hard guards only.** Program code enforces target scope and dry-run non-mutation: editable local/unprotected skills, `USER.md`, and `MEMORY.md` are allowed; everything else is out of scope. Do not add confidence gates, canary modes, approval queues, or safety-heavy policy layers unless Ryo explicitly reopens that design.
...
5. **Prefer Hermes-native tool surfaces over bespoke loops.** Use `AIAgent(enabled_toolsets=...)` and optional tool-name whitelist before adding custom LLM tool-call loops or validators. Existing plugin-owned tool-call replay/parser/dispatcher loops are technical debt unless they provide a proven post-validation or accounting guarantee that Hermes' native surface cannot provide.
6. **Trace, logs, and post-state beat prose.** Natural-language finalizer claims are not sufficient. Tool trace, old_text/before-after details, and read-back state must drive accounting and make manual revert possible.
...
- [LLM model routing spec and memory extractor role](2026-05-22-llm-model-routing-spec-and-memory-extractor.md) — implemented. It formalizes which model config key each LLM site uses, adds `model.memory_extractor` as a first-class tool-free role, strengthens tests that DSPy/GEPA use `model.evaluator`, updates docs so constrained tool-using roles are not confused with tool-free auxiliary/DSPy calls, and adds non-breaking requested model/provider telemetry fields.
- [Pre-release role simplification](2026-05-22-pre-release-role-simplification.md) — implemented. It removed unreleased compatibility rather than preserving it: injected editor loops and synthetic finalizer support were deleted from skill/memory backend source, tests now use constrained-runner/helper coverage for that path, a guard keeps the removed synthetic finalizer name out of active source/tests/docs, status now prints runtime setup reasons while keeping `setup` as the only init/repair surface, and `target_resolver` is now a thin runtime-private overlay target while remaining mutation-free.
- [Constrained LLM role tooling and resolver handoff](2026-05-21-target-resolver-light-tuning-and-planner-handoff.md) — implemented through constrained default editor paths. It defines and tests the minimal tool permission matrix: `target_resolver` and `improvement_planner` get read-only `skills_list` / `skill_view`; `skill_agent` gets skill mutation tools; `memory_agent` gets the memory mutation tool; evaluator / memory_extractor / GEPA stay tool-free with host-prepared context. Current progress: shared tool permissions, native tool-harness helper extraction, AIAgent message trace recovery, constrained adapters/default routing for both editor backends, auxiliary fallback removal, final-JSON active prompts, and runtime setup recovery are done.
...
Goal: report cross-surface canonical transactions clearly while preserving minimal hard guards, exact `old_text` checks, add-before-remove placement moves, and unified run summaries.
