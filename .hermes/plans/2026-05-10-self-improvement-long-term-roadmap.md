...
- [Pre-release role simplification](2026-05-22-pre-release-role-simplification.md) — removes unreleased compatibility rather than preserving it. Task 1 is implemented: injected editor loops and synthetic finalizer support were deleted from skill/memory backend source, and tests now use constrained-runner/helper coverage for that path. Remaining work is to remove the synthetic `submit_mutation_result` name from active tests/docs/prompts, keep `setup` as the single runtime init/repair surface with clearer status guidance, and add `target_resolver` as a thin runtime-private overlay target while keeping resolver mutation-free and planner-owned for create/mutate decisions.
