...
- Slice 1 visibility is implemented and dogfooded: latest dry-run artifact exposes matched-but-not-selected counts and memory-routed-to-skill drops instead of hiding them under generic benign skips. Slice 2 has moved the runtime planner producer/consumer contract to canonical `knowledge_transactions`, and run artifacts now expose memory-to-skill cross-store candidates in the same top-level transaction list. Memory-to-skill execution now has a single add-before-remove transaction result, episode/CLI/tool summary surfaces consume canonical transactions when present, planner-owned cross-store selections are counted, selected coverage representatives are handled, active runtime producers now emit `memory_convert_to_skill_update` instead of the old terminal workflow-skip reason, unified editor-backend errors are normalized to `editor_*`, and final `run_improve` artifacts expose canonical `step_decisions.knowledge_transactions` / `knowledge_routing` summaries without legacy split `step_decisions.skill` / `memory` / `memory_to_skill` lanes.
