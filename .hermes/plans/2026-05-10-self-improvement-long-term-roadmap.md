...
- Follow-up `2026-06-01-memory-placement-cron-hardening.md` is implemented and verified for the latest cron finding: `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260531T190325Z.json` selected two USER↔MEMORY placement applies but both blocked before mutation with `knowledge_transaction_missing_required_fields` because execution required `content` despite exact `source_old_text`. The fix makes placement moves default destination content to `source_old_text`, preserves add-before-remove, validates dry-run apply transactions through the executor boundary, and surfaces blocked apply transactions in reporting. Verification: focused related suite `135 passed`; full suite `939 passed, 2 skipped`; `py_compile` and `git diff --check` clean. Dogfood dry-run `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260601T000243Z.json` stayed dry-run/no mutation with `placement_move apply=3`, `blocked_count=0`, and raw planner diagnostics selected `memory_to_skill=3`, so no immediate separate planner-stability plan was opened.
