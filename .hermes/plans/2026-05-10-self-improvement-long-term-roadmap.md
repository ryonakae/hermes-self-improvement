...
- Slice 1 visibility is implemented and dogfooded: latest dry-run artifact exposes matched-but-not-selected counts and memory-routed-to-skill drops instead of hiding them under generic benign skips. Slice 2 has moved the runtime planner producer/consumer contract to canonical `knowledge_transactions`, and run artifacts now expose memory-to-skill cross-store candidates in the same top-level transaction list. Memory-to-skill execution now has a single add-before-remove transaction result, and episode/CLI/tool summary surfaces consume canonical transactions when present. The remaining blocker is planner-owned cross-store transaction selection plus complete removal of split prompt/report/readiness schema leakage.
...
- **Knowledge inventory beyond tool failures:** around 7.6合目.
  ...
  - Milestone 4 phase 4.2 adds deterministic placement routing inside `reconcile_memory_extractor_payload_with_existing_memories`: raw tool output → `skip_reason="not_memory_raw_tool_output"` + `suggested_route="diagnostic"`, procedural / step-shaped facts → `skip_reason="not_memory_workflow_to_skill"` + `suggested_route="skill"`.
