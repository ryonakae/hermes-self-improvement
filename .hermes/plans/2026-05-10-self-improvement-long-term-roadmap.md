...
- [LLM model routing spec and memory extractor role](2026-05-22-llm-model-routing-spec-and-memory-extractor.md) — planned. It formalizes which model config key each LLM site uses, adds `model.memory_extractor` as a first-class tool-free role, strengthens tests that DSPy/GEPA use `model.evaluator`, and updates docs so constrained tool-using roles are not confused with tool-free auxiliary/DSPy calls.
