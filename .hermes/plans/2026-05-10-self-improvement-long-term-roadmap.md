...
- **Knowledge inventory beyond tool failures:** around 7.2合目.
  ...
  - Milestone 4 phase 4.2 adds deterministic placement routing inside `reconcile_memory_extractor_payload_with_existing_memories`: raw tool output candidates (code fences / stdout-stderr / shell prompts) get `skip_reason="not_memory_raw_tool_output"` + `suggested_route="diagnostic"`, and procedural / step-shaped facts get `skip_reason="not_memory_workflow_to_skill"` + `suggested_route="skill"`. The existing `Memory placement:` CLI section already aggregates duplicate / kept / would-move / would-merge-replace / routed-to-skill / diagnostic / needs-memory-planner counts. Repo/runtime drift (4.3) still needs expansion.
