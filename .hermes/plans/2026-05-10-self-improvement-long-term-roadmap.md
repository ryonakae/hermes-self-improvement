...
- [Pre-release role simplification](2026-05-22-pre-release-role-simplification.md) — removes unreleased compatibility rather than preserving it. Tasks 1-2 are implemented: injected editor loops and synthetic finalizer support were deleted from skill/memory backend source, tests now use constrained-runner/helper coverage for that path, and a guard keeps the removed synthetic finalizer name out of active source/tests/docs. Remaining work is to keep `setup` as the single runtime init/repair surface with clearer status guidance, and add `target_resolver` as a thin runtime-private overlay target while keeping resolver mutation-free and planner-owned for create/mutate decisions.
