...
- `2026-05-19_101746-local-skill-lifecycle-expansion.md`
  - **Status:** in progress; Tasks 1-6 implemented.
  - Expands skill lifecycle handling from narrow `agent_created_report()` candidates to all Hermes-changeable local unprotected skills. Implemented slices add path-aware `$HERMES_HOME/skills/` inventory, protect built-in/hub/vendor/plugin/external/pinned/archived/ambiguous skills, feed editable local skills into planner/target-resolution candidate lists, block duplicate `create_skill` when an existing local unprotected skill covers the workflow, carry merge/absorb semantics through `skill_agent` with source/successor target validation and successor post-validation, and add deterministic active-reference rewrite planning for cron/config/local skill Markdown before archive. Remaining work: applying rewrites and archive execution after rewrites, lifecycle reporting, and dogfood cleanup of the existing duplicate.
