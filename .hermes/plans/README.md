...
Active planning follow-up (2026-06-02): `2026-06-02-semantic-review-actionability-followup.md` is planned and not yet implemented. It follows the completed semantic review plan and targets two dry-run gaps from `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260601T164953Z.json`: 8 `memory_to_skill` selections blocked with `transaction_missing_source_evidence_id`, and 9 raw `placement_split` decisions deferred because exact split text was missing. The follow-up keeps semantic judgment in the Planner while preserving explicit source ids, hardening templates, requiring exact `destination_content` / `source_replacement` for executable split transactions, and re-dogfooding with route leak checks.
