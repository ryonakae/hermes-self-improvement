...
Update note (2026-06-01): `2026-06-01-memory-placement-cron-hardening.md` is implemented and verified. It responds to cron artifact `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260531T190325Z.json`, where two valid-looking USER↔MEMORY `placement_move` applies stopped safely with `knowledge_transaction_missing_required_fields` because the executor required `content` even though the planner supplied exact `source_old_text`. The fix makes `placement_move` use `source_old_text` as default destination content, preserves add-before-remove, routes dry-run apply transactions through executor validation, and reports blocked apply transactions explicitly instead of hiding them as no-op. Verification: focused related suite `135 passed`; full suite `939 passed, 2 skipped`; `py_compile` and `git diff --check` clean. Dogfood dry-run `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260601T000243Z.json` stayed dry-run/no mutation (`target_changed=False`), had `placement_move apply=3`, `blocked_count=0`, and raw planner diagnostics selected `memory_to_skill=3`; no immediate separate planner-stability plan was opened.
