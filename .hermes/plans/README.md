...
- `2026-05-21-target-resolver-light-tuning-and-planner-handoff.md`
  - **Status:** partially implemented; role permission matrix, constrained role runner, resolver/planner read-only skill-tool routing, editor backend tool-surface constant unification, shared native tool-harness helper extraction, AIAgent message trace recovery, and thin constrained adapters for both skill-agent and memory-agent backends are done. Remaining: make the constrained paths the normal editor execution paths after trace/finalizer accounting proof.
