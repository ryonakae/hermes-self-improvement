...
Active planning follow-up (2026-06-01): `2026-06-01-llm-semantic-knowledge-judgment.md` is the next detailed implementation plan. Phases 1–5 are **complete**: canonical transaction schema (placement_split, memory_rewrite, duplicate_cleanup, keep_same_topic_different_store, skill_ambiguity_cleanup), evidence builder relation candidates, planner digest/prompt templates, dry-run artifact reporting with bounded semantic counts, and safe Knowledge Editor execution for memory_rewrite + duplicate_cleanup + placement_split. Phase 6 (existing skill-first + ambiguity cleanup) and Phase 7 (golden fixture / dogfood quality gate) remain pending. Latest verification: 263 tests passed across all related suites.
