...
- `2026-05-21-target-resolver-light-tuning-and-planner-handoff.md`
  - **Status:** partially implemented; role permission matrix, constrained role runner, resolver/planner read-only skill-tool routing, editor backend tool-surface constant unification, shared native tool-harness helper extraction, AIAgent message trace recovery, and the first thin constrained skill-agent adapter are done. Remaining: make the constrained path the normal skill editor execution path after trace/finalizer accounting proof, then repeat for memory.
