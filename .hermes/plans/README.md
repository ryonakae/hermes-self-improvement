...
Active follow-up (2026-06-01): `2026-06-01-memory-placement-direction-validation.md` is planned and independently reviewed after mutating replay exposed a root planner/normalizer bug, not just an executor safety issue. The known bad candidate `memory_place_2217e1ddb538` had `current_store=user`, `suggested_route=likely_keep`, and exact old_text `日本語docsは日本語中心、英語は必要時のみ。`, but the raw planner/normalizer path produced an executable-looking `move_memory_to_user` / `builtin_memory -> builtin_user` transaction with empty `evidence_ids`. The plan fixes the LLM-facing prompt and digest so only store-valid move directions are offered, then adds a context-aware normalizer gate that validates placement direction against candidate `current_store`, preserves evidence ids, and reports bounded invalid-direction diagnostics. First review blocked planner-runtime-only validation and missing public-path regression; the plan was patched. Second review PASS. Implementation has not started yet; begin with Task 1 RED.
