...
Update note (2026-05-30): `2026-05-29-unified-knowledge-editor-memory-inventory.md` Slices 1–3 are implemented. Built-in memory inventory now flows into planner evidence/digest as first-class `built_in_memory_inventory` rows from the official `MemoryStore` / runtime current-entry path, the planner prompt renders those entries with exact `old_text`, and planner output can emit canonical built-in memory placement/cleanup transactions (`placement_move`, `memory_replace`, `memory_delete`, `memory_to_skill`, or `none`). The next active implementation slice is Slice 4: execute cross-surface transactions through the unified Knowledge Editor using official skill/memory tools.
...
- `2026-05-29-unified-knowledge-editor-memory-inventory.md`
  - **Status:** active; Slices 1–3 implemented, Slice 4 next.
