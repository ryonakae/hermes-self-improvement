...
Active follow-up (2026-06-01): `2026-06-01-memory-placement-heuristic-minimalization.md` was implemented and verified in this worktree. It supersedes the marker-heavy `suggested_route` tuning line: semantic USER/MEMORY/Skill placement is decided by the Planner LLM from exact text, current store, official boundaries, and neutral observations, while programmatic code keeps only destructive-operation hard guards. The implementation removed planner-facing `suggested_route` / `likely_*` values, route-priority prompt ordering, route-hint canonical-store cleanup, and route-named memory placement diagnostics in run artifacts. It also preserved the direction/source/stale/add-before-remove guards from `2026-06-01-memory-placement-direction-validation.md`. Verification: full suite `949 passed, 2 skipped`; source-directed dry-run artifact `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260601T113151Z.json` has zero memory-placement route terms/diagnostics.
