...
Active follow-up (2026-06-02): `2026-06-02-llm-led-memory-capacity-recovery.md` is planned and not yet implemented. It follows the mutating replay artifact `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260602T055107Z.json`, where six canonical USER→MEMORY `placement_move` applies were semantically plausible but all blocked safely with `memory_capacity_exceeded`. The plan keeps capacity recovery judgment in the LLM Planner / Knowledge Editor: program code may expose bounded facts, hard safety guards, official memory-tool execution, and verification only; it must not choose which memory to compact/remove/replace by heuristic. Capacity follow-ups must feed a path that actually calls Planner (`run_improve` / explicit replan), not plain `--from-run` replay. Completion requires artifact-visible capacity follow-up records, planner prompt context, canonical LLM-selected resolution execution, compact blocked/resolved reporting, and dogfood proving either safe LLM-led mutation or explicit LLM defer/skip.
