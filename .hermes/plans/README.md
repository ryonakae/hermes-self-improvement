...
Update note (2026-05-31): `2026-05-31-memory-placement-priority-planner-tuning.md` is implemented through related-suite and source dry-run verification, with full-suite verification pending in this cycle. It follows the default-defer diagnostics slice by making `likely_memory_to_skill` placement candidates a priority prompt section, giving both a `memory_to_skill` template when exact `target_skill` is known and an evidence-id-preserving `defer` template when target skill is unclear, and sorting the full placement list so procedural candidates precede low-action `likely_keep`. Source dry-run `/Users/ryo.nakae/.hermes/self-improvement/runs/run-20260530T222529Z.json` stayed dry-run/no mutation and improved placement compliance to `planner_decision_count=25`, `default_defer_count=0`, `unhandled_count=0`, `default_defer_by_route={}`. Safety and execution gates are unchanged.
