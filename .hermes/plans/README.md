...
Update note (2026-05-30): `2026-05-29-unified-knowledge-editor-memory-inventory.md` Slices 1–4 are implemented. Built-in memory inventory now flows into planner evidence/digest as first-class `built_in_memory_inventory` rows from the official `MemoryStore` / runtime current-entry path, the planner prompt renders those entries with exact `old_text`, planner output can emit canonical built-in memory placement/cleanup transactions, and the Knowledge Editor executes skill, built-in memory, placement move, and memory-to-skill transactions through official tool paths. Placement moves add destination before removing source, preserve the source on add/capacity failure, retry after same-target capacity recovery, and record compact tool results plus rollback hints. The next active implementation slice is Slice 5: report memory inventory and cross-surface edits clearly.
...
- `2026-05-29-unified-knowledge-editor-memory-inventory.md`
  - **Status:** active; Slices 1–4 implemented, Slice 5 next.
