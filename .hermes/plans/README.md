...
Active plan (2026-06-07): `2026-06-07-planner-owned-knowledge-decision-and-editor-execution.md` is the current follow-up from the 2026-06-04 cron/dogfood discussion. Goal: make the Planner's USER/MEMORY/Skill and capacity decisions match a human Hermes reviewer, then make the Knowledge Editor execute that concrete plan through Hermes official tools without independent semantic override. It explicitly treats capacity, compaction, split, memory-to-skill, duplicate cleanup, and whole-move permission as Planner-owned semantic decisions; Editor remains LLM-based but is an executor/patch composer with mechanical guards only. First implementation slice is a deterministic golden fixture for recent Ryo memory examples from `run-20260603T190347Z.json`, before fixing the built-in memory execution path, so bad whole-entry placement moves do not start mutating just because provider/tool routing is repaired.
