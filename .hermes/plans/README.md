...
Update note (2026-05-29): the next active implementation slice is `2026-05-29-unified-knowledge-editor-memory-inventory.md`. It codifies the simplified product model Ryo chose: one Planner judges across skills, `USER.md`, and `MEMORY.md`; one Knowledge Editor improves across skills and memory through official `skill_manage` and `memory` tools. No new role, approval queue, confidence gate, or safety-heavy lane should be added. Minimal hard guards only: editable local/unprotected skills plus `USER.md`/`MEMORY.md`; dry-run must not mutate; logs/artifacts must retain enough old_text/before-after detail to manually revert bad edits.
