...
`2026-06-17-report-accounting-and-outcome-credit-hardening.md` is the current implementation plan. Phase 1 fixed the 2026-06-17 operational mismatch where immediate `improve` output correctly reported `skill patched 3 / memory 3`, while later `report` / daily artifact rendered `Recent runner artifacts` as `skill patched 0 / memory 3`. Phase 2 is also implemented: conservative outcome reporting now carries `unknown_reasons` and renders an `unknown breakdown` when new credit-assignment data includes it, without counting silence as success. Phase 3 is implemented as well: new episodes and outcome observations carry bounded matching signatures, and broad generic recurrence clusters stay diagnostic-only instead of becoming recurrence proof. Phase 4 is implemented: runtime eval cases now carry source signature/outcome metadata and prefer high-signal outcome cases over weak-usage cases. Verification passed: Phase 4 related suite `34 passed`, runtime eval focused suite `16 passed`, full suite `1037 passed, 2 skipped`, `py_compile`, `git diff --check`, status/report smoke, and source-directed runtime eval smoke passed. Remaining planned work is Phase 5 only: end-to-end verification and dogfood.
