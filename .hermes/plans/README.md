...
- `2026-05-19_101746-local-skill-lifecycle-expansion.md`
  - **Status:** in progress; Tasks 1-5 implemented.
  - Expands skill lifecycle handling from narrow `agent_created_report()` candidates to all Hermes-changeable local unprotected skills. Implemented slices add path-aware `$HERMES_HOME/skills/` inventory, protect built-in/hub/vendor/plugin/external/pinned/archived/ambiguous skills, feed editable local skills into planner/target-resolution candidate lists, block duplicate `create_skill` when an existing local unprotected skill covers the workflow, and carry merge/absorb semantics through `skill_agent` with source/successor target validation, source-mutation guards, `merged_from` / `archive_candidates` result fields, and successor post-validation. Remaining work: active reference rewrite planning, archive execution after rewrites, lifecycle reporting, and dogfood cleanup of the existing duplicate.
