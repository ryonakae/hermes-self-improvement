...
- Started / completed the plugin-local YAML config slice from Task 1.5. Added tracked `config.example.yaml`, gitignored local `config.yaml` / `config.local.yaml` / `config.local.json`, extended config loading to support JSON and YAML with precedence `defaults < config.json < config.yaml < config.local.json < config.local.yaml < HERMES_SELF_IMPROVE_CONFIG < --config`, recursive `${ENV}` expansion, fail-closed explicit env/CLI config parsing, deep nested config merge, and compatibility normalization from legacy `llm_scorer` / `gepa_scorer` model fields into `model.llm` / `model.gepa`. Runtime LLM scoring now reads `model.llm`, DSPy program construction reads `model.gepa`, and GEPA compile artifacts include redacted model config summaries. Added config precedence tests for YAML, env expansion, unresolved variables, explicit YAML paths, legacy normalization, gitignore, and parseable example config.
- Continued the Hermes-authenticated DSPy LM bridge slice. `dspy_program.py` now builds a DSPy `BaseLM` adapter that routes scoring calls through `agent.auxiliary_client.call_llm(...)`, using plugin-local `model.gepa` fields (`provider`, `model`, `base_url`, `api_key`, `timeout`, `max_tokens`, `extra_body`) and returning an OpenAI-chat-shaped response for DSPy. The real DSPy scoring program runs `dspy.Predict` under `dspy.context(lm=...)` when the DSPy runtime supports it, while fake-DSPy tests remain dependency-free. `gepa-optimize` now passes `model.gepa` into the student program and, when available, into `dspy.GEPA(reflection_lm=...)`; GEPA config summaries continue to redact secrets. Added fake tests for auxiliary-client routing, DSPy LM context use, and optimizer `reflection_lm` wiring.
