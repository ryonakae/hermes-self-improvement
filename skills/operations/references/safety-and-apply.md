...
- `evaluator_promote` approvals bind candidate id/path/hash, regression result hash, active evaluator pointer path, active-before pointer hash, and rollback strategy. Validation must reject candidate drift or active pointer drift before apply.
...
4. `memory_delete` targets must resolve under configured `memory_roots` and point at an existing file.
5. Reject absolute names, `..`, and root escapes.
6. Do not infer target files from prose titles or natural-language proposal text.
...
- supports `delete_created_file` rollback for approved `skill_create` ledgers, before-snapshot restore for approved `skill_delete` / `memory_delete` ledgers, `rename_file_back` for approved `skill_rename` ledgers, and `restore_multiple_files` for approved `skill_merge` ledgers
