...
- Planner quality should be judged from proof counts, not selected count alone. Watch `attached_candidate_count`, `unmatched_evidence_count`, `selected_with_evidence`, `action_like_skips`, target-hint attachment counts/match kinds, and editor prompt chars in dry-run output/artifacts. If target hints attach nearly everything, inspect match kinds before trusting the planner selection.
