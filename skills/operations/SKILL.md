...
- `hermes_self_improvement/mutation_worker.py`: tool-mediated mutation executor。skill mutation は `skill_manage` の許可 action のみ実行可。built-in memory は `memory` tool、Hindsight stale correction は `hindsight_retain` のみ。直接 fallback はしない。
