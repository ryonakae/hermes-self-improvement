...
- Curator と self-improvement は代替関係ではない。Curator は skill usage / pinned state / `active → stale → archived` lifecycle / optional consolidation を担当する。self-improvement は tool failure、memory operation/failure、user correction/session outcome、subagent outcome、LLM/API failure metadata を証拠に、skill / memory / user profile / evaluator prompt の改善を担当する。
...
- 定期運用では built-in Curator を `hermes curator pause` で一時停止し、2つの autonomous maintainer が同じ skill library を別々に変更しないようにする。`curator.enabled: false` にはしない。pause が止めるのは scheduled Curator run だけで、config / telemetry / pin / lifecycle state / CLI management は残る。Curator 単独運用へ戻すときは `hermes curator resume`。
- `improve` は paused 中も Curator/Hermes telemetry を skill candidate の source of truth として読む。mutating `improve` は candidate load 前に Curator と同じ `apply_automatic_transitions()` を実行することがある。pause を「Curator の情報やライフサイクル機能を捨てる設定」と説明しない。
