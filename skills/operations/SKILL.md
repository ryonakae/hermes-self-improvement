...
6. 実 mutation slice を追加するときは preview-first を崩さない。low-risk apply は explicit confirmation と expected item hash、approved apply は approval hash + target hash + rollback preview/post-write validation、retention prune は expected artifact list hash、whole-file replacement は full before snapshot rollback を必須にする。`skill_create` / `skill_delete` のような file lifecycle mutation は approval-gated path だけで扱い、create は missing target + rollback delete、delete は existing target + before snapshot restore を必須にする。新しい destructive / broad mutation は low-risk に混ぜず、approval-gated path から始める。
