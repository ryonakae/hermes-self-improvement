...
- `improve` は skill / memory 変更を episode として append-only に記録する。`calibrate` は前回 `calibrate` 以降の観測から outcome observation を作り、明示的に紐づく episode だけを採点する。unmatched observation は artifact に残し、scoring には使わない。
