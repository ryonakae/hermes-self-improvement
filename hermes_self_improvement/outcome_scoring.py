...
def build_outcome_score_aggregate(*, config: dict[str, Any], limit: int = 1000) -> dict[str, Any]:
    ...
    for row in scored:
        by_planner[str(row.get("planner_prompt_hash") or "unknown")].append(row)
    ...
    aggregate = {
        ...
        "by_planner_prompt_hash": {key: _bucket_summary(value) for key, value in sorted(by_planner.items())},
