...
def merge_verifier_status(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        from .skill_agent_backend import _ensure_hermes_agent_on_path

        _ensure_hermes_agent_on_path()
        import agent.auxiliary_client  # type: ignore  # noqa: F401
    except Exception as exc:
        return {"available": False, "reason": "hermes_auxiliary_unavailable", "detail": str(exc), "model_source": "model.skill_agent"}
    return {"available": True, "source": "hermes_auxiliary", "model_source": "model.skill_agent"}
