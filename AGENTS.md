...
- built-in memory mutation は `memory` tool のみを呼びます。外部 memory mutation は provider policy / context の dry-run resolution までで、実行は fail-closed / needs_review に留めます。
